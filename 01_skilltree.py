...
def solution(skill, skilltree):
    skill_stack=[]
    res_count=0
    if (skill.isupper()==False):
        return 'incorrect input'
    for word in skill:
        skill_stack.append(word)
    for num in range(len(skilltree)):
        res = 1
        skill_count=0
        for word2 in skilltree[num]:
            if word2.islower()==True:
                return 'incorrect input'
            if word2 in skill_stack:
                if skill_stack[skill_count]==word2:
                    res=1
                    skill_count+=1
                else:
                    res=0
...
result=solution('CBD',['BACDE','CBADF','AEBC','BDA'])
print(result)
